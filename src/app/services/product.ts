...
export class ProductService {
  ...
  private products: ProductLister[] = [
    {
      id: 1,
  ...
  getProducts(): ProductLister[] {
    return this.products;
  }

  // getProductById(id: number): ProductLister | undefined {
  //   return this.products.find(p => p.id === id);
  // }

  getInStockProducts(): ProductLister[] {
    return this.products.filter(p => p.is_in_inventory);
  }

  getOutOfStockProducts(): ProductLister[] {
    return this.products.filter(p => !p.is_in_inventory);
  }
  ...
  getInStockCount(): number {
    return this.products.filter(p => p.is_in_inventory).length;
  }
